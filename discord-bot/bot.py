...
# Không yêu cầu Privileged Intents (Message Content) để bot chạy ngay mà không cần bật cài đặt trên Discord Portal
...
def fetch_devices():
    try:
        ...
        with urllib.request.urlopen(req, timeout=4) as response:
            ...
            # Tự động dọn dẹp các máy rác/mất kết nối quá 45 giây khỏi Firebase
            now = time.time() * 1000
            active_data = {}
            for dev_id, dev in data.items():
                if isinstance(dev, dict):
                    last_active = dev.get("lastActive", 0)
                    if (now - last_active) < 45000:  # Chỉ giữ lại máy online trong 45s qua
...
def send_bot_heartbeat():
    try:
        ...
        print(f"Lỗi khi gửi heartbeat: {e}")

def send_command(target_id, action):
    try:
...
class ControlPanelView(discord.ui.View):
    def __init__(self, devices_data):
        ...
        # 1. Global Buttons
        btn_run_all = discord.ui.Button(label="⚡ Chạy Tất Cả", style=discord.ButtonStyle.primary, custom_id="btn_global_run", row=0)
        btn_pause_all = discord.ui.Button(label="⏸ Tạm Dừng Tất Cả", style=discord.ButtonStyle.secondary, custom_id="btn_global_pause", row=0)
        btn_resume_all = discord.ui.Button(label="▶ Tiếp Tục Tất Cả", style=discord.ButtonStyle.success, custom_id="btn_global_resume", row=0)
        btn_stop_all = discord.ui.Button(label="⏹ Dừng Tất Cả", style=discord.ButtonStyle.danger, custom_id="btn_global_stop", row=0)
        btn_refresh = discord.ui.Button(label="🔄", style=discord.ButtonStyle.secondary, custom_id="btn_refresh", row=0)

        btn_run_all.callback = self.make_global_callback("RUN_NOW", "CHẠY VÒNG LẶP")
        btn_pause_all.callback = self.make_global_callback("PAUSE", "TẠM DỪNG")
        btn_resume_all.callback = self.make_global_callback("RESUME", "TIẾP TỤC")
        btn_stop_all.callback = self.make_global_callback("STOP", "DỪNG HẲN")
        btn_refresh.callback = self.refresh_callback

        self.add_item(btn_run_all)
        self.add_item(btn_pause_all)
        self.add_item(btn_resume_all)
        self.add_item(btn_stop_all)
        self.add_item(btn_refresh)

        # 2. Per-Device Buttons (Tối đa 4 máy)
        for idx, dev in enumerate(online_devices[:4]):
            ...
            btn_dev_run = discord.ui.Button(label=f"⚡ Chạy [M{idx+1}]", style=discord.ButtonStyle.primary, row=row_num, disabled=(is_running and not is_paused))
    ...
    def make_global_callback(self, action, name):
        async def callback(interaction: discord.Interaction):
            send_command("global", action)
            await interaction.response.send_message(f"⚡ **LỆNH TẤT CẢ:** Đã gửi lệnh **{name}** tới TOÀN BỘ máy!", ephemeral=True)
        return callback

    def make_device_callback(self, dev_id, action, name):
        async def callback(interaction: discord.Interaction):
            send_command(dev_id, action)
            await interaction.response.send_message(f"🎯 Đã gửi lệnh **{name}** tới máy `{dev_id}`!", ephemeral=True)
        return callback
...
def build_panel(devices_data=None):
    ...
    embed = discord.Embed(
        ...
        description="Hệ thống giám sát và điều khiển từ xa các máy đang chạy Chrome Extension.",
    ...
    if not online_devices:
        ...
        for idx, dev in enumerate(online_devices):
            ...
            status_icon = "🟢 ĐANG CHẠY" if status == "running" else ("🟡 ĐANG TẠM DỪNG" if status == "paused" else "⚪ SẴN SÀNG")
    ...
    embed.set_footer(text="TPlus Cloud Controller • Tự động dọn dẹp và cập nhật mỗi 5 giây")
...
async def auto_update_panels():
    ...
    to_remove = []
    for msg_id, (channel_id, _) in active_panels.items():
        try:
